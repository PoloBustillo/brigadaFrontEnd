...
export default function BrigadistaHome() {
  ...
  const fetchAssignments = async (showLoading = true) => {
    ...
    if (!isOnline) {
      ...
      setIsLoading(false);
      if (!hasAnyAssignments) setFetchError(true);
      return;
    }
    ...
    try {
      ...
      // Cache write should never block UI loading state.
      void cacheRepository.set(CACHE_KEY, data, CACHE_TTL).catch((err) => {
        console.warn("Error caching assignments:", err);
      });
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      await Promise.all([fetchAssignments(), loadBottomBarConfig()]);
      if (!mounted) return;
      await loadLocalStats();
    };

    bootstrap().catch((err) => {
      console.error("Error bootstrapping home:", err);
  ...
  const handleBottomMenuAssignmentPress = (
    ...
    router.push({
      ...
      params: {
        ...
        questionsJson: JSON.stringify(assignment.latest_version.questions ?? []),
      },
    });
  };
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <ScrollView
        ...
        contentContainerStyle={[
          ...
          {
            paddingBottom:
              contentPadding + (bottomBarMenuItems.length > 0 ? 96 : 0),
          },
        ]}
        refreshControl={
          <RefreshControl
            ...
            fetchError ? (
              ...
              >
                ...
                <Text style={[styles.emptyStateTitle, { color: colors.error }]}>
                  {isOnline ? "No se pudieron cargar encuestas" : "Sin conexión"}
                </Text>
                <Text
                  style={[
                ...
                >
                  {isOnline
                    ? "No se pudo cargar la información. Toca para reintentar."
                    : "No se pudo cargar. Toca para reintentar."}
                </Text>
              </TouchableOpacity>
            ) : (
